...
9. TNN PREDICTION VALIDATION (NEW)
   - When TNN model outputs are provided (tnn_clase, tnn_probabilidades):
     * Assess physical plausibility against current atmospheric conditions
     * Compare prediction against climatological expectations for location/season
     * Check for consistency between clase (classification) and probabilidades (probabilities)
     * Evaluate if prediction aligns with known synoptic patterns
     * Flag unlikely combinations (e.g., high "Fuerte" probability but low "Moderada")
   - Provide confidence in prediction reliability:
     * High confidence: prediction aligns with atmospheric physics & climatology
     * Medium confidence: consistent with data but some uncertainty remains
     * Low confidence: prediction contradicts known processes or is atypical
   - Recommend whether to trust the prediction for operational decisions
...
EXPERT_QUERY_TEMPLATES = {
    ...
    "tnn_validation": """
    Validate the TNN model prediction provided (clase={tnn_clase}, probabilidades={tnn_probabilidades}, 
    horizonte={tnn_horizonte}h).
    
    Is this prediction physically plausible given:
    - Current atmospheric conditions and synoptic patterns?
    - Known climatology for this season and location?
    - Consistency between the class prediction and probability distribution?
    
    If alerta={tnn_alerta} (alert flag), is heavy precipitation truly justified?
    
    Provide confidence level (High/Medium/Low) in the prediction reliability 
    and recommend whether to trust this output for operational decisions.
    """,
}
