...
try:
    ...
    HAS_NCPS = False
    print("Warning: ncps library not available. Liquid Neural Networks will not work.")
...
class ConvLSTMCell(nn.Module):
    """ConvLSTM cell for processing spatio-temporal data."""
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size=3):
        ...
        # Convolutional layers for input-to-state
        self.conv_gates = nn.Conv2d(
            input_channels + hidden_channels, 2 * hidden_channels,
    ...
    def forward(self, input_tensor, cur_state):
        ...
        combined_candidate = self.conv_candidate(
            torch.cat([input_tensor, reset_gate * h_cur], dim=1)
        )
        candidate = torch.tanh(combined_candidate)
        
        c_next = (1 - update_gate) * c_cur + update_gate * candidate
...
class ConvLSTMClassifier(nn.Module):
    """ConvLSTM-based classifier for spatio-temporal precipitation data."""
    ...
    def forward(self, x):
        ...
        for _ in range(self.num_layers):
            h_states.append(torch.zeros(batch_size, self.hidden_channels, h, w, 
                                       device=x.device, dtype=x.dtype))
            c_states.append(torch.zeros(batch_size, self.hidden_channels, h, w, 
                                       device=x.device, dtype=x.dtype))
        ...
        for t in range(seq_len):
            ...
            for layer_idx, cell in enumerate(self.convlstm_cells):
                ...
                x_t = h_states[layer_idx]  # Output of layer i is input to layer i+1
                x_t = F.dropout(x_t, p=self.dropout_rate, training=self.training)
        
        # Use final hidden state
        final_state = h_states[-1]  # (batch, hidden_channels, h, w)
        pooled = self.global_pool(final_state)  # (batch, hidden_channels, 1, 1)
        pooled = pooled.view(pooled.size(0), -1)  # (batch, hidden_channels)
        
        logit = self.classifier(self.dropout(pooled))
        return logit.squeeze(-1)


class LiquidNeuralNetworkClassifier(nn.Module):
    """Liquid Neural Network (LTC-based) for time-series classification."""

    def __init__(self, input_dim, hidden_dim=128, dropout=0.3):
        super().__init__()
        if not HAS_NCPS:
            raise RuntimeError(
                "ncps library is required for Liquid Neural Networks. "
    ...
    def forward(self, x):
        ...
        # LTC processes sequences directly
        hidden_states = self.ltc(x)  # (batch, seq_len, hidden_dim)
        
        # Use the final hidden state
        final_hidden = hidden_states[:, -1, :]  # (batch, hidden_dim)
...
def main():
    ...
    # Add Liquid Neural Network if available
    if HAS_NCPS:
        models['LNN'] = LiquidNeuralNetworkClassifier(n_features, hidden_dim=args.hidden_dim).to(device)
        print("  ✓ Liquid Neural Network (LNN) added")
